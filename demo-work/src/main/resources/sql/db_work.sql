# 数据库
create database if not exists db_work;

# 任务表
create table db_work.tb_work
(
    ...
    constraint tb_work_work_pk
        unique (work_id)
);

# 任务日志表
create table db_work.tb_work_log
(
    ...
    create_time  datetime     null comment '任务创建时间',
    update_time  datetime     null comment '任务更新时间',
    constraint tb_work_work_log_pk
        unique (log_id)
);